...
class DeepfakeDataset(Dataset):
    def __init__(self, root_dir, transform=None, max_videos_per_class=10):
        face_folder = os.path.join(root_dir, "numpy_faces")
        self.samples = []
        self.video_to_faces = {}
    ...
    def __getitem__(self, idx):
        ...
        img = np.load(npy_path).astype(np.float32)   # <-- ensure float32
        if self.transform:
            img = self.transform(img)
        img = img.float()                             # <-- ensure tensor is float32
...
def main():
    ...
    for epoch in range(3):
        ...
        for imgs, blink, labels in train_loader:
            imgs, blink, labels = imgs.to(device).float(), blink.to(device), labels.to(device)  # <-- float conversion
            optimizer.zero_grad()
